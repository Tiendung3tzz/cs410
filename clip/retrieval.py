...
def load_embedding():
  ...
  loaded_embeddings = np.load('../embeding/image_embeddings.npy')
  model = SentenceTransformer('clip-ViT-B-32')
  image_path = "/content/drive/MyDrive/colab/cs336/data/avengre"
  image_files = glob (os.path.join(image_path, "*.png"))
  return loaded_embeddings, model, image_path,image_files
st.cache_resource
def get_clip():
...
def visualize_results(query, distances, retrieved_images):
  ...
  for col, img_path, distance in zip(cols, retrieved_images, distances):
      with col:
          ...
          st.image(img, caption=f"Dist: {distance:.2f}", use_column_width=True)
